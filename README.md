...
### Inbox / Outbox и семантика exactly once

В проекте реализованы паттерны **Outbox** и **Inbox**, обеспечивающие надёжное асинхронное взаимодействие между микросервисами и семантику *exactly once* на уровне бизнес-операций.

**Outbox**:

* В Orders Service и Payments Service используется таблица `outbox_messages`
* Все события (например запрос на списание средств или результат оплаты) сначала сохраняются в базу данных в рамках одной транзакции вместе с изменением бизнес-состояния
* Фоновый процесс (OutboxDispatcher) читает сообщения из outbox и публикует их в RabbitMQ
* Это гарантирует, что событие не потеряется даже при сбое сервиса после коммита транзакции

**Inbox**:

* При получении сообщений из RabbitMQ сервис проверяет, обрабатывалось ли сообщение ранее
* Повторная обработка одного и того же сообщения не приводит к повторному изменению состояния
* Это защищает систему от повторных доставок сообщений брокером

**Семантика exactly once**:

* Достигается за счёт комбинации:

    * транзакционного сохранения данных и событий (Outbox)
    * идемпотентной обработки входящих сообщений (Inbox)
* Даже если сообщение будет доставлено несколько раз, бизнес-операция (например списание денег или изменение статуса заказа) будет выполнена только один раз
